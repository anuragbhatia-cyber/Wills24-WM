  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {(() => {
        ...
        const wonCount = effectiveStatusCounts['won'] ?? 0
        const lostCount = effectiveStatusCounts['lost'] ?? 0
        const activePipeline =
          (effectiveStatusCounts['new'] ?? 0) +
          (effectiveStatusCounts['assigned'] ?? 0) +
          (effectiveStatusCounts['follow-up'] ?? 0) +
          (effectiveStatusCounts['quotation-sent'] ?? 0) +
          (effectiveStatusCounts['projected'] ?? 0) +
          (effectiveStatusCounts['invoice-sent'] ?? 0)
        ...
        return (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {cards.map((c) => (
              <div
                key={c.label}
                className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-xs dark:shadow-none px-4 py-3.5"
              >
